...
//Receiving index as an argument
const getPath = (index) => {
switch (index) {
  case 0:
    return "/app";
...
const Sidebar = () => {
  const dispatch = useDispatch(); //video 22
    ...
    <Box
      ...
      {Nav_Buttons.map((el) => (
        ...
        onClick={()=> {
          ...
          navigate(getPath(el.index));//The result of this is the path
        }}
      ...
      <Avatar id="basic-button"
        ...
        onClick={handleClick} src={faker.image.avatar()} sx={{cursor : "pointer"}}/>
      <Menu
        id="basic-menu"
        anchorEl={anchorEl}
        open={open}
